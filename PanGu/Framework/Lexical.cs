...
namespace PanGu.Framework
{
        ...
        public override void DoThings(int action, DFA<int, LexicalFunction> dfa)
        {
            ...
            switch (Func)
            {
                ...
                case LexicalFunction.OutputSpace:
                    ...
                    break;
                case LexicalFunction.OutputEndOfLine:
                    lexical.OutputToken = new WordInfo();
                    GetTextElse(dfa);
                    lexical.OutputToken.WordType = WordType.EndOfLine;
                    break;
    ...
    public class Lexical:DFA<int, LexicalFunction>
    {
        ...
        private static void InitSpaceStates()
        {
            ...
            //s0 [ \t\r\n] s3
            s0.AddNextState(new int[]{' ', '\t'}, s3.Id);

            //s3 [ \t\r\n] s3
            s3.AddNextState(new int[] { ' ', '\t'}, s3.Id);
        ...
        private static void InitEndOfLineStates()
        {
            DFAState<int, LexicalFunction> s10 = AddState(new LexicalState(10, false)); //EOL begin state;
            DFAState<int, LexicalFunction> s11 = AddState(new LexicalState(11, true, LexicalFunction.OutputEndOfLine)); //EOL quit state;

            //s0 [ \t\r\n] s3
            s0.AddNextState(new int[] {'\r', '\n'}, s10.Id);

            //s3 [ \t\r\n] s3
            s10.AddNextState(new int[] { '\r', '\n' }, s10.Id);

            //s3 ^[ \t\r\n] s4
            s10.AddElseState(s11.Id);
        }
